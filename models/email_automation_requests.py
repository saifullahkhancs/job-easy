...
class EmailAutomationRequest(Base):
    ...
    reviewed_by_admin_email = Column(String, ForeignKey("users.email"), nullable=True)
    admin_notes = Column(Text, nullable=True)

    # Relationships
    user = relationship("User", back_populates="automation_requests", foreign_keys=[user_email])
    reviewed_by_admin = relationship("User", foreign_keys=[reviewed_by_admin_email])
